    ...
    <div className="space-y-4">
      ...
      <div>
        ...
        <select value={values.tier_id} disabled={tiersLoading} onChange={e=>onChange({ tier_id: e.target.value })}
          className={cn('w-full px-4 py-3 rounded-xl border border-gray-700 bg-black','text-white focus:ring-2 focus:ring-ios-primary focus:border-pink-500')}>
      ...
      <div>
        ...
        <input type="text" value={values.account_level} onChange={e=>onChange({ account_level: e.target.value })}
          className={cn('w-full px-4 py-3 rounded-xl border border-gray-700 bg-black','text-white placeholder-ios-text-secondary','focus:ring-2 focus:ring-ios-primary focus:border-pink-500')} placeholder="e.g., Level 50, Master Rank, Diamond" />
      </div>
      <div>
        ...
        <textarea rows={4} value={values.account_details} onChange={e=>onChange({ account_details: e.target.value })}
          className={cn('w-full px-4 py-3 rounded-xl border border-gray-700 bg-black','text-white placeholder-ios-text-secondary','focus:ring-2 focus:ring-ios-primary focus:border-pink-500')} placeholder="Additional account information, items included, etc." />
      </div>
    </div>
  );
};
