...
const PaymentStatusPage: React.FC = () => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-ios-background text-white px-6">
  ...
  if (!order) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-ios-background text-white px-4 with-bottom-nav">
        <div className="text-center">
          <h2 className="text-xl sm:text-2xl font-bold text-white mb-4">Tidak ada detail order</h2>
          <p className="text-white-secondary mb-6 text-sm sm:text-base">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            ...
            <Link 
              to="/" 
              className="border border-gray-700 px-6 py-3 min-h-[44px] rounded-xl bg-black text-white hover:bg-black/80 transition-colors text-center"
            >
              Beranda
            </Link>
          </div>
  ...
  return (
    <div className="min-h-screen bg-ios-background text-white with-bottom-nav">
      <div className={standardClasses.container.boxed}>
        <div className="bg-black border border-gray-700 rounded-2xl p-6 sm:p-8 shadow-lg">
          <h1 className={`text-xl sm:text-2xl font-bold mb-4 ${style.color}`}>{style.title}</h1>
          
          <div className="space-y-4 mb-8">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
              <span className="text-white-secondary text-sm">Order ID:</span>
              <span className="font-mono text-sm sm:text-base">{order.id}</span>
            </div>
            
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center">
              <span className="text-white-secondary text-sm">Total Pembayaran:</span>
              <span className="font-bold text-lg text-white">Rp {Number(order.amount).toLocaleString('id-ID')}</span>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <Link 
              to="/products" 
              className="bg-pink-500 text-white px-6 py-3 min-h-[44px] rounded-xl font-semibold hover:bg-pink-500/90 transition-colors text-center"
            >
              Lanjut Belanja
            </Link>
            <Link 
