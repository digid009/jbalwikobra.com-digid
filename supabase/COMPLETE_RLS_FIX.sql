...
DROP POLICY IF EXISTS "flash_sales_public_select" ON public.flash_sales;
...
DROP POLICY IF EXISTS "game_titles_public_select" ON public.game_titles;
...
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'tiers') THEN
    ...
    EXECUTE 'DROP POLICY IF EXISTS "tiers_public_select" ON public.tiers';
...
DROP POLICY IF EXISTS "rental_options_public_select" ON public.rental_options;
...
DROP POLICY IF EXISTS "product_likes_public_select" ON public.product_likes;
...
DROP POLICY IF EXISTS "feed_post_likes_public_select" ON public.feed_post_likes;
